...
class TopDownGenerateTargetFewShot:
    ...
    def _msra_generate_target(self, cfg, joints_3d, joints_3d_visible, sigma):
        ...
        target_xy = np.zeros((num_joints, H, W), dtype=np.float32)
        ...
        if self.unbiased_encoding:
            ...
            for joint_id in range(num_joints):
                ...
                if target_weight[joint_id] > 0.5:
                    ...
                    target_xy[joint_id][int((img_y[0]+img_y[1])/2)][int((img_x[0]+img_x[1])/2)]=1
                    #target_xy[joint_id][1]=mu_y
        ...
        return target, target_weight,target_xy
        ...
        if self.encoding == 'MSRA':
            if isinstance(self.sigma, list):
                ...
                for i in range(num_sigmas):
                    target_i, target_weight_i,target_xy_i = self._msra_generate_target(
                    ...
                    target_xy= np.concatenate(
                        [target_xy, target_xy_i[None]], axis=0)
            else:
                target, target_weight,target_xy = self._msra_generate_target(
